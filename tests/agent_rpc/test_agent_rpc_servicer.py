...
class AgentRpcServicerTests(unittest.TestCase):
    ...
    def test_review_multi_agent_file_input_passes_parsed_text_and_json_safe_document_to_supervisor(
        ...
        self.assertEqual(initial_input["document_metadata"]["page_count"], 0)
        self.assertIsNone(initial_input["document_blocks"][0]["location"]["page_no"])
        self.assertIsNone(initial_input["clause_chunks"][0]["page_no"])
        self.assertIsNone(initial_input["rag_documents"][0]["metadata"]["page_no"])
        self.assertIsNone(initial_input["evidence_json"]["blocks"][0]["location"]["page_no"])
        self.assertIsNone(initial_input["evidence_json"]["chunks"][0]["page_no"])
        self.assertIn("page=?", initial_input["llm_context"])

...
graph TB
    ...
    subgraph 核心层
        ...
        MODEL_CFG[Model Config<br/>config/]
    end
...
### 4.3 `contract_agent/config/` — YAML 运行时与角色化模型配置

**职责**: 统一 `.run/config.yaml` 运行时配置，按角色（chat / embedding / rerank）分离模型配置，并支持环境变量部署 overlay 与本地 CLI profile overlay。
...
| 文件 | 内容 |
|------|------|
| `config_app.py` | `AppConfig`, `AppContext` and adapters to runtime/model/retrieval/multi-agent config |
| `config_loader.py` | YAML loader for `.run/config.yaml`, profile overlay and runtime installation |
| `config_runtime.py` | `Settings` runtime snapshot and explicit runtime injection helpers |
| `config_model.py` | `ModelRole`, `ModelEndpointConfig`, `ModelRuntimeConfig`, `ModelConfigSource`, `ModelProfileStore`, `DEFAULT_PROVIDER_OPTIONS` |
| `config_model_store.py` | environment/static/YAML model sources, profile codec/store, resolver and service factory |

**配置加载顺序**: 代码默认 `AppConfig` → `.run/config.yaml` → 环境变量 overlay → CLI Profile overlay（仅覆盖 `models.*`）。
...
### 8.2 环境变量 overlay 优先级

```text
角色专用变量 > 统一 LLM 变量 > Qwen 兼容变量 > YAML/代码默认值
...
`.run/cli_profile.yaml` 生命周期：

```text
1. contract-agent demo → 交互式向导
2. 向导收集 chat/embedding/rerank 的 provider, base_url, api_key, model
3. 保存到 .run/cli_profile.yaml
4. 后续 CLI 命令通过 configure_runtime() 自动加载 profile
...
```text
代码默认值 (AppConfig / Settings 默认值)
    ↓ 被覆盖
.run/config.yaml (主运行时配置)
    ↓ 被覆盖
环境变量 overlay (CHAT_PROVIDER, LLM_API_KEY, AGENT_GRPC_PORT, ...)
    ↓ 被覆盖
CLI Profile (.run/cli_profile.yaml，仅覆盖 models.*)
```
...
`logger/audit.py` 中的 `AuditLogger` 写入结构化 JSONL 到 `.run/audit.jsonl`。它不再只是审查审计日志，而是运行时 debug 链路：

- `trace_id`: 单次请求或流水线的链路 ID，gRPC → Service → RAG/Agent 会复用同一个 ID
- `span_id` / `parent_span_id`: 阶段级嵌套 span，用于定位耗时和失败位置
- `prefix`: 来源定位前缀，例如 `[RPC]`、`[Service][Review]`、`[Service][Chat]`、`[Knowledge][RAG]`、`[Orchestration][Supervisor]`
- `duration_ms`: trace/span 完成或失败时的耗时
- `error_type` / `error`: 失败事件的异常类型和消息

```jsonl
{"event":"trace.started","prefix":"[RPC]","scope":"rpc","operation":"grpc.Review","trace_id":"...","timestamp":"..."}
{"event":"span.started","prefix":"[Service][Review]","span_name":"review.rules","trace_id":"...","span_id":"...","timestamp":"..."}
{"event":"review.rules.completed","prefix":"[Service][Review]","contract_type":"采购合同","risk_count":8,"trace_id":"...","span_id":"...","timestamp":"..."}
{"event":"span.completed","prefix":"[Knowledge][RAG]","span_name":"rag.rerank","candidate_count":12,"duration_ms":84.2,"trace_id":"...","span_id":"...","timestamp":"..."}
{"event":"trace.completed","prefix":"[RPC]","operation":"grpc.Review","duration_ms":682.5,"trace_id":"...","timestamp":"..."}
```
...
```powershell
# 按请求链路追踪
Select-String -Path .run/audit.jsonl -Pattern '"trace_id":"<id>"'

# 按模块定位
Select-String -Path .run/audit.jsonl -Pattern '\\[Knowledge\\]\\[RAG\\]'
Select-String -Path .run/audit.jsonl -Pattern '\\[RPC\\]'
```
...
**启动**: `contract-agent-grpc` loads `.run/config.yaml` by default. The gRPC port defaults to `50051` from YAML (`grpc.port`) and can still be overlaid by `AGENT_GRPC_PORT` for process-local overrides. Programmatic entrypoints may pass `config_path` explicitly; CLI and RAG utility commands expose `--config`.
...
```text
1. 欢迎横幅 → 检查 .run/cli_profile.yaml
...
**现状**: 当前测试套件包含 76 个 unittest 用例，覆盖规则审查、CLI、配置、Provider、Rerank、持久化边界、schema 初始化、后台追踪日志、服务配置注入、RAG eval 配置注入、Gateway 路由、Supervisor ReAct 循环、RAG 检索链路、gRPC servicer 直接调用和包结构约束。
...
已为 `provider`、`config`、`runtime`、`memory`、`knowledge`、`review`、`services`、`agents`、`schemas`、`knowledge.rag.rerank` 等关键扩展包补充显式 re-export；其中重依赖包使用懒导出，避免导入公共 API 时拉起 LangChain、Redis、SQLAlchemy 等重依赖。
...
| `.run/config.yaml` | - | Primary runtime config file for server, model, retrieval and multi-agent settings |
| `AGENT_GRPC_PORT` | `50051` | Environment overlay for gRPC port (`grpc.port`) |
...
├── config/
│   ├── __init__.py                      # 配置聚合出口
│   ├── config_app.py                    # AppConfig/AppContext and YAML section adapters
